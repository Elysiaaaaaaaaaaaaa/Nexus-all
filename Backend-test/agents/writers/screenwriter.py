...
class ScreenWriter:
    ...
    def deal_screen(self,raw_screen):
        """
...
def test_deal_screen():
    """
    测试 deal_screen 函数是否能正确分割分镜
    """
...
分镜 3：信中写着令人震惊的消息。''',
        ...
        {
            'name': '包含斜杠的测试',
            'input': '''分镜 1：主角在街上行走，突然停了下来/他看到了一个熟悉的身影。
分镜 2：那个身影转过身，竟然是他的老朋友。
分镜 3：两人相拥而泣，场面感人。''',
            'expected_count': 3
        },
        {
            'name': '数字格式不统一',
            'input': '''分镜1：镜头从天空俯瞰整个城市。
分镜 2：镜头快速下移，聚焦到一栋高楼。
分镜3：镜头穿过窗户，进入房间内部。''',
            'expected_count': 3
        },
...
字幕开始出现''',
    ...
    print("=" * 50)
    print("开始测试 deal_screen 函数")
    print("=" * 50)
    
    for i, test_case in enumerate(test_cases, 1):
        print(f"\n测试用例 {i}: {test_case['name']}")
        print("-" * 50)
        result = screenwriter.deal_screen(test_case['input'])
        print(f"预期分镜数量: {test_case['expected_count']}")
        print(f"实际分镜数量: {len(result)}")
        print(f"测试结果: {'通过' if len(result) == test_case['expected_count'] else '失败'}")
        print("\n分割结果:")
        for j, screen in enumerate(result, 1):
            print(f"\n分镜 {j}:")
            print(screen)
