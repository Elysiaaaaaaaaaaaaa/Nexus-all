...
const Interaction = () => {
  ...
  useEffect(() => {
    // 处理后端的修改确认状态
    if (sessionData?.now_state === 'modify_confirm' || sessionData?.now_state === 'modify_comfirm') {
  ...
  const sendToBackend = async (payload) => {
    ...
    try {
      const response = await workflowAPI.processWork(projectName, content, 'production');
  ...
  const submitModifyDecision = async ({ needModify }) => {
    ...
    if (!needModify) {
      ...
      setIsSending(true);
      try {
        const resp = await sendToBackend({ content: '不需要修改', modify_num: [] });
        let aiText = normalizeAiContent(resp?.message || resp?.reply?.text || resp?.response || '');
        if (!aiText || aiText.trim() === '') {
